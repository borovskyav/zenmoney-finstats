...
from client.transaction import PostCreateIncomeRequestBody
from finstats.domain import AccountId, InstrumentId, Merchant, TagId, Transaction, UserId
...
class IncomeTransactionsController(BaseController):
    ...
    @apispec.json_schema(mr.schema(PostCreateIncomeRequestBody))
    ...
    async def post(self) -> web.StreamResponse:
        request = await self.parse_request_body(PostCreateIncomeRequestBody)
    ...
    @staticmethod
    def validate_request_body(body: PostCreateIncomeRequestBody) -> None:
        if body.amount <= 0:
