...
class TagsRepository:
    ...
    async def get_tag(self, tag_id: TagId) -> ZmTag | None:
        stmt = sa.select(TagTable).where(TagTable.id == tag_id)
        async with self.__connection_scope.acquire() as connection:
            result = await connection.execute(stmt)
            return to_dataclass(ZmTag, result.one_or_none())

    async def get_tags(self) -> list[ZmTag]:
        stmt = sa.select(TagTable)
        async with self.__connection_scope.acquire() as connection:
            result = await connection.execute(stmt)
            return to_dataclasses(ZmTag, result.all())

    async def get_children_tags(self, parent_tag_id: TagId) -> list[ZmTag]:
        stmt = sa.select(TagTable).where(TagTable.parent == parent_tag_id)
        async with self.__connection_scope.acquire() as connection:
            result = await connection.execute(stmt)
            return to_dataclasses(ZmTag, result.all())

    async def get_tags_by_id(self, tag_ids: list[TagId]) -> list[ZmTag]:
        if not tag_ids:
            return []
    ...
    async def save_tags(self, tags: list[ZmTag]) -> None:
        if not tags:
            return
